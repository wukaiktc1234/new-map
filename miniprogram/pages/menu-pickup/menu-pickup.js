...
Page({
  ...
  onLoad() {
    ...
    app.setCurrentOrderType('pickup')
    this.setData({
  ...
  generateTimeOptions() {
    ...
    const options = [{ label: '尽快取餐', value: 'asap', desc: '约15-20分钟' }]
  ...
  updateCartInfo() {
    this.setData({
      cartCount: app.getCartCount('pickup'),
      cartTotal: app.getCartTotal('pickup').toFixed(2)
    })
  },
  ...
  selectTime(e) {
    ...
    if (option) {
      this.setData({
        selectedTime: value,
        pickupTime: value === 'asap' ? '尽快取餐' : option.label,
        showPicker: false
      })
    }
  },
  ...
  goToCart() {
    wx.navigateTo({
      url: '/pages/cart/cart?type=pickup'
    })
  },
  ...
  goToConfirm() {
    ...
    wx.navigateTo({
      url: '/pages/order-confirm/order-confirm?type=pickup&pickupTime=' + encodeURIComponent(this.data.pickupTime)
    })
  }
})
