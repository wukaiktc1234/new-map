...
| ENV-3 | **mvn 默认 JAVA_HOME 指向不存在的 JDK 路径**：`mvn`（`/h/fuwu/apache-maven-3.9.11`）内置 `JAVA_HOME=H:\fuwu\jdk-17.0.17+10`，该路径不存在（H 盘/JDK 已迁移）→ 默认编译直接失败；须显式覆盖（实测 `JAVA_HOME=/h/jdk-25.0.1.8-hotspot` Temurin 25.0.1 编译 BUILD SUCCESS） | WIP 诊断 `docs/quality/workspace-wip-diagnosis-001.md` §6（2026-09-25，Owner 指令） | 低（开发环境；不阻断仓库/CI 事实，仅本地编译需手动覆盖） | 否 | 观察（修复 mvn 配置或更新 JAVA_HOME 为本地动作，待 Owner 处理） |
| ENV-1 状态修正 | **ENV-1 所指 WIP 已被 `aec5c45` 吸收入库，不再以工作区形式存在**：原 `PosOrderCreateServiceImpl` +681 行未提交 WIP（FOODID 卡 §V9 登记）随 aec5c45（该文件 +767/-?）入库；当前该文件对 HEAD 无真实内容 diff（仅行尾幻影，2026-09-25 幻影清理后 status 已干净）。ENV-1 的历史事实与登记不删改，仅状态修正为 **ABSORBED_BY_aec5c45**；其"开卡未清理工作区"教训由 ENV-2 + PG-001 承接 | WIP 诊断 `docs/quality/workspace-wip-diagnosis-001.md` §5（2026-09-25） | — | 否 | 状态修正（FOODID 卡文档为已收口文档不改，仅此处登记） |
...
*追加：2026-09-25 WIP 诊断之 ENV 登记（来源：Owner 指令 + `docs/quality/workspace-wip-diagnosis-001.md` 只读诊断）：主表新增两行——**ENV-3**（mvn 默认 JAVA_HOME 指向不存在的 `H:\fuwu\jdk-17.0.17+10`，编译须显式覆盖，实测 Temurin 25 可用）与 **ENV-1 状态修正**（原 +681 行 WIP 已被 `aec5c45` 吸收入库 → ABSORBED_BY_aec5c45，FOODID 卡文档不删改）。同日完成**行尾幻影清理**：1196 个 M 中 730 个为 autocrlf 行尾噪声（内容与 HEAD 逐字节一致），经 stash 备份（8 个 stash 条目在册）+ checkout 清除，M 数 1196 → **466**（= 真实内容改动数，残余幻影 0）。主表 81 → 83 行（ENV-2 已计入 81）。ENV 序列独立编号，不占 KL。planner 仅登记与诊断，未 stash 恢复、未 discard、未 commit 工作区 WIP；历史条目零修改。*

*追加：2026-09-25 登记 KL-081（163 处 schema 对齐债）：来源 P0-SCHEMA-SINGLE-SOURCE-001 首跑 `scripts/check-schema-alignment.py`，分类 MISMATCH=93 / NO-TABLE=70 / NO-ENTITY=57（分解见 `schema-alignment-baseline-20260925-v2.txt`；v1 已标 SUPERSEDED）。归因：历史建表只存在于 DatabaseFixConfig、未走 Flyway；归档遗留脚本误导实体编码。**既有债，非登记卡引入**；处置：随涉表/实体卡片按 PG-003 逐个消化，不一次性修复（P0-SCHEMA 卡禁止改实体）。关联：P1-POS-MENU-500-001（FoodCategory 已修复 0 错位）。主表 83 → 84 行。planner 仅登记，未修错位、未改实体、未改 Flyway、未 commit 业务代码；历史条目零修改。*

*追加：2026-09-25 P2-GOV-IGNORE-EVIDENCE-001 升级 P1 + 第二次触发登记（来源：P0-SCHEMA-SINGLE-SOURCE-001 的基线 `.txt` 落盘被 `.gitignore` 全局 `*.txt` 规则拦截）：`schema-alignment-baseline-20260925(-v2).txt` 均需 `git add -f` 绕过，**累计触发 2 次**（第一次：2026-09-24 `c5cec24` 26 个 evidence `.txt`；第二次：本次 2 个基线 `.txt`）。**升级 P2 → P1**：拦截对象已从 evidence 目录扩大到 `docs/quality/` 治理产物，绕过动作从一次性变为常态化。**修复建议（待排期，非本次执行）**：`.gitignore` 的 `*.txt` 规则加豁免（如 `!docs/quality/*.txt`、`!docs/architecture/**/evidence/**/*.txt`）或改为定向忽略清单。原 2026-09-24 登记注记为历史条目零修改；本条为升级记录。planner 仅登记，未改 `.gitignore`、未动 git 历史。*
