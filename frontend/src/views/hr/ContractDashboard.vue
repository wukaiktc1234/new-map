...
/* ===== 统计卡片 ===== */
const statsCards = computed<Array<{ icon: string; label: string; value: number; colorType: 'primary' | 'success' | 'warning' | 'error' | 'info' }>>(() => [
  { icon: 'AlarmClock', label: '已过期', value: expiryStats.value.expired, colorType: 'error' },
  { icon: 'Warning', label: '7天内到期', value: expiryStats.value.urgent, colorType: 'warning' },
  { icon: 'Calendar', label: '30天内到期', value: expiryStats.value.warning, colorType: 'primary' },
  { icon: 'Bell', label: '90天内到期', value: expiryStats.value.notice, colorType: 'info' },
])
...
function goToTemplateLibrary() {
  router.push('/hr/contract-template')
}
...
<template>
  <div class="modern-page">
    <PageHeader title="合同智能管理" description="到期提醒 · 续签推荐 · 风险识别 · 模板管理">
      <el-button @click="goToTemplateLibrary">合同模板库</el-button>
      <el-button type="primary" @click="goToContractManagement">合同管理</el-button>
    </PageHeader>
      ...
      <el-tab-pane label="到期合同提醒" name="expiry">
        ...
        <el-table v-loading="loading" :data="filteredExpiring" stripe size="small">
          ...
          <el-table-column label="操作" width="80" fixed="right">
            <template #default>
              <el-button link type="primary" size="small" @click="goToContractManagement">处理</el-button>
            </template>
          </el-table-column>
          <template #empty>
