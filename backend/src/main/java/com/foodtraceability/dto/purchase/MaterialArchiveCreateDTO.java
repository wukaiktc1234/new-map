...
/**
 * 商品档案创建 DTO
 * 对应前端 MaterialArchiveFormData
 */
@Schema(description = "商品档案创建请求")
public class MaterialArchiveCreateDTO {

    /** 商品名称 */
    @NotBlank(message = "商品名称不能为空")
    @Size(max = 200, message = "商品名称长度不能超过200字")
    @Schema(description = "商品名称", example = "番茄", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    /** 参考价（分）—— 由前端 Converter 元转分后传给后端 */
    @NotNull(message = "参考价不能为空")
    @Min(value = 0, message = "参考价不能为负数")
    @Schema(description = "参考价（分）", example = "500", requiredMode = Schema.RequiredMode.REQUIRED)
